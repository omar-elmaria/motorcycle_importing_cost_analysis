...
with open(file=os.path.join(os.getcwd(), "motorcycle_crawling", "product_page.json"), mode="r") as json_file:
...
for i in data:
    ...
    time.sleep(2) # Wait for 1 second until the action is executed
    ...
    i.update({"calculated_price": re.findall(pattern="(.*),", string=calculated_price)[0].replace(" ", "")})
    print(data[0])
    break
